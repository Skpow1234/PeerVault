...
func (f *Formatter) formatFileListCSV(files []FileInfo) (string, error) {
	...
	// Header
	_ = writer.Write([]string{"ID", "Name", "Size", "Hash", "Created", "Owner"}) // Ignore error for demo purposes

	// Rows
	for _, file := range files {
		_ = writer.Write([]string{ // Ignore error for demo purposes
			file.ID,
			file.Name,
...
func (f *Formatter) formatPeerListCSV(peers []PeerInfo) (string, error) {
	...
	// Header
	_ = writer.Write([]string{"ID", "Address", "Status", "Latency", "Storage", "Last Seen"}) // Ignore error for demo purposes
